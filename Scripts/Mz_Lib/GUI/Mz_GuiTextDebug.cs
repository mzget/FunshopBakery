    ...
    void OnGUI () {
        ...
        if (debugIsOn) {
			...
			GUI.matrix = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, 
				new Vector3(Screen.width / Main.GAMEWIDTH, Screen.height / Main.GAMEHEIGHT, 1));

            GUI.BeginGroup (new Rect(windowPosition, 40.0f, leftSide, 200.0f));
			{
                scrollViewVector = GUI.BeginScrollView (new Rect (0, 0.0f, debugWidth, 200.0f), scrollViewVector, 
