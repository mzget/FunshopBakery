...
//		base.WaitForIngredient(ingredientName);		
		...
		if(ingredientName == JamBeh.StrawberryJam) {
			...
			this.gameObject.name = GoodDataStore.GoodsOrderList.toast_with_strawberry_jam.ToString();
		}
		else if(ingredientName == JamBeh.BlueberryJam) {
			base.animatedSprite.Play(JamBeh.BlueberryJam);
			
			this.gameObject.name = GoodDataStore.GoodsOrderList.toast_with_blueberry_jam.ToString();
		}
		else if(ingredientName == JamBeh.ButterJam) {
			base.animatedSprite.Play(JamBeh.ButterJam);
			
			this.gameObject.name = GoodDataStore.GoodsOrderList.toast_with_butter_jam.ToString();
		}
		else if(ingredientName == JamBeh.CustardJam) {
			base.animatedSprite.Play(JamBeh.CustardJam);
			
			this.gameObject.name = GoodDataStore.GoodsOrderList.toast_with_custard_jam.ToString();
		}
